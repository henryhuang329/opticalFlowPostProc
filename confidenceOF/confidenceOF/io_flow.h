...
public:
  ...
  png::image<png::rgb_pixel> errorImage (FlowImage &F_noc,FlowImage &F_occ) {
...
private:
  ...
  void writeFlowField (const std::string file_name) {
    ...
    for (int32_t v=0; v<height_; v++) {
      for (int32_t u=0; u<width_; u++) {
        ...
        val.blue  = 0;
        if (isValid(u,v)) {
          val.red   = (uint16_t)std::max<float>(std::min<float>(getFlowU(u,v)*64.0f+32768.0f,65535.0f),0.0f);
          val.green = (uint16_t)std::max<float>(std::min<float>(getFlowV(u,v)*64.0f+32768.0f,65535.0f),0.0f);
          val.blue  = 1;
        }
  ...
  inline float hsvToRgb (float h, float s, float v, float &r, float &g, float &b) {
    float c  = v*s;
    float h2 = 6.0*h;
    float x  = c*(1.0-fabs(fmod(h2,2.0f)-1.0));
    ...
    for (int32_t v=0; v<height_; v++) {
      for (int32_t u=0; u<width_; u++) {
        ...
        if (isValid(u,v)) {
          ...
          float s   = std::min<float>(std::max(mag*n/max_flow,0.0f),1.0f);
          float v   = std::min<float>(std::max(n-s,0.0f),1.0f);
