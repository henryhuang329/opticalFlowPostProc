...
int predictConfidence(const char* testData, string outDir, string curName)
{
  ...
  string fileOut = outDir+"\\confData\\"+curName+".data";
  const  char* outName = fileOut.data();
  char* bufOut = new char[strlen(outName)+1];
  strcpy(bufOut, outName);
  
  argv[1] = "-l";
  argv[2] = "C:\\NvidiaCode\\HenryCModel\\164543_TV_class.xml"; // Load Generated Random Forest Model.

  char *buf = new char[strlen(testData)+1];
  strcpy(buf, testData);
